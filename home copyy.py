...
# Add this function to load content based on checkbox selections
def load_content_based_on_selection():
    # Check which checkboxes are selected
    # if st.session_state.get("show_recent_images", False):
    if st.sidebar.checkbox("Show Recent Images"):
        # Load recent images
    ...
    if st.sidebar.checkbox("Show Logs files"):
        # Load and display logs files
        ...
        # Display logs files content
        for key, (filename, file_url) in logs_user_files.items():
    ...
    if st.sidebar.checkbox("Show Cache files"):
        # Load and display cache files
        ...
        for user, file_url in cache_user_files.items():
    ...
    if st.sidebar.checkbox("Show Config files"):
        ...
        for user, file_url in config_user_files.items():
    ...
    if st.sidebar.checkbox("Show Keylogerror files"):
        # Load and display keylogerror files
        ...
        for user, file_url in keylogerror_user_files.items():
    ...
    with tab1:
        ...
        def dashboard():
            ...
            if user_data:
                ...
                # st.sidebar.checkbox("Show Recent Images", key="show_recent_images", on_change=load_content_based_on_selection)
                
                
                # Call the function to load content based on the current checkbox states
                load_content_based_on_selection()
