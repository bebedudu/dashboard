...
# GITHUB_TOKEN = "add_your_token_here"  # Replace with your actual GitHub token
...
#     # Sort files by date and time extracted from the filename
#     filtered_files.sort(key=lambda x: datetime.strptime(x["name"][:15], "%Y%m%d_%H%M%S"), reverse=True)
#     return filtered_files  # Return all files sorted by timestamp
...
#             filtered_files = filter_files_for_user(files, st.session_state.unique_id)

#             # User input for number of images
#             num_images = st.number_input("Number of Images to Display", min_value=1, max_value=len(filtered_files), value=10)
#             filtered_files = filtered_files[:num_images]

#             # Display images in a grid layout
#             if filtered_files:
#                 st.write(f"Displaying {len(filtered_files)} Images:")
...
#                             st.image(image_url, caption=file["name"])  # Smaller image size
#             else:
#                 st.info("No images found for this user.")
#         else:
#             st.warning("No files available in the repository.")
...
#         # Fetch files from GitHub
#         files_images = fetch_files_from_github(GITHUB_API_URL_IMAGES)
#         files_logs = fetch_files_from_github(GITHUB_API_URL_LOGS)

#         if files_images or files_logs:
#             # Filter files for the logged-in user
#             filtered_files_images = filter_files_for_user(files_images, st.session_state.unique_id)
#             filtered_files_logs = filter_files_for_user(files_logs, st.session_state.unique_id)

#             # Display images in a grid layout
#             if filtered_files_images:
#                 st.write("Latest Images:")
#                 cols_per_row = 4  # Number of images per row
#                 for i in range(0, len(filtered_files_images), cols_per_row):
#                     row_files = filtered_files_images[i:i + cols_per_row]
#                     cols = st.columns(cols_per_row)
#                     for col, file in zip(cols, row_files):
#                         with col:
#                             image_url = file["download_url"]
#                             st.image(image_url, caption=file["name"], width=150)  # Smaller image size
...
#                         json_content = safe_parse_json(file_content)
...
#         files_images = fetch_files_from_github(GITHUB_API_URL_IMAGES)
#         files_logs = fetch_files_from_github(GITHUB_API_URL_LOGS)
#         files_config = fetch_files_from_github(GITHUB_API_URL_CONFIG)

#         if files_images or files_logs or files_config:
#             # Filter files for the logged-in user
#             filtered_files_images = filter_files_for_user(files_images, st.session_state.unique_id)
#             filtered_files_logs = filter_files_for_user(files_logs, st.session_state.unique_id)
#             filtered_files_config = filter_files_for_user(files_config, st.session_state.unique_id)
...
#         files_logs = fetch_files_from_github(GITHUB_API_URL_LOGS)
#         files_config = fetch_files_from_github(GITHUB_API_URL_CONFIG)
#         files_keylogerror = fetch_files_from_github(GITHUB_API_URL_KEYLOGERROR)

#         if files_images or files_logs or files_config or files_keylogerror:
#             # Filter files for the logged-in user
#             filtered_files_images = filter_files_for_user(files_images, st.session_state.unique_id)
#             filtered_files_logs = filter_files_for_user(files_logs, st.session_state.unique_id)
#             filtered_files_config = filter_files_for_user(files_config, st.session_state.unique_id)
#             filtered_files_keylogerror = filter_files_for_user(files_keylogerror, st.session_state.unique_id)
...
#                 st.write("Keylog Error Files:")
#                 for file in filtered_files_keylogerror:
#                     st.write(file["name"])
#                     file_content = get_file_content(file["download_url"])
#                     if file["name"].endswith(".log"):
#                         st.text_area("File Content", file_content, height=200)
#                     st.write("---")  # Separator between keylogerror files
...
# # Function to fetch files from GitHub
# @st.cache  # Use @st.cache for caching
# def fetch_files_from_github(api_url):
#     headers = {"Authorization": f"token {GITHUB_TOKEN}"}
#     response = requests.get(api_url, headers=headers)
#     if response.status_code == 200:
...
#             num_images = st.number_input("Number of Images to Display", min_value=1, max_value=len(sorted_images), value=20)
...
# DEFAULT_TOKEN = "asdfgadd_your_token_hereaefthu"
...
#                 sorted_images = sort_image_files_by_timestamp(filtered_files_images) 
#                 # Input box for the number of images to display
#                 num_images = st.sidebar.number_input("Number of Images to Display", min_value=1, max_value=len(sorted_images), value=10)
#                 # Limit the images to the specified number
#                 limited_images = sorted_images[:num_images]
...
#         © {current_year} Active User Dashboard | Developed by Bibek Chand Sah
