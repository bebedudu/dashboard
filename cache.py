...
REPO_URL = "https://api.github.com/repos/bebedudu/keylogger/contents/uploads/cache"
...
#         filename = file['name']
#         match = re.match(r"(?:\d{8}_\d{6}_)?(.+?)_files_cache\.json", filename)
#         if match:
...
#             st.write(f"**{user}**: [Download File]({file_url})")
...
#                     st.download_button(label="Download File", data=content, file_name=f"{user}_files_cache.json", mime="application/json")
...
#     st.text_area("", st.session_state["fullscreen_content"], height=600)
...
# add the open page content in new page
# Function to fetch file list from GitHub
def get_github_files():
    headers = {"Authorization": f"token {GITHUB_API_KEY}"}
    response = requests.get(REPO_URL, headers=headers)
