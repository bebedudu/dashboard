...
def delete_files(folder, num_files_to_delete):
    st.write(f"Deleting {num_files_to_delete} files from {folder}...")
    for i in range(num_files_to_delete):
        time.sleep(1)  # Simulate deletion delay
        st.write(f"Deleted file {i + 1} from {folder}")
    st.success(f"Successfully deleted {num_files_to_delete} files from {folder}!")
...
def dashboard():
    ...
    # Create tabs
    tab1, tab2 = st.tabs(["Count Files", "Delete Files"])
    ...
    with tab2:
        ...
        # Display checkboxes and sliders for each folder
        for folder_name, folder_path in FOLDERS.items():
            st.subheader(f"Folder: {folder_name}")
            num_files = get_number_of_files(folder_path)

            # Checkbox to enable/disable deletion for this folder
            enable_deletion = st.checkbox(f"Enable deletion for {folder_name}", key=f"enable_{folder_name}")

            if enable_deletion:
                # Slider to select the number of files to delete
                ...
                if st.button(f"Delete Files from {folder_name}", key=f"delete_{folder_name}"):
                    if num_files_to_delete > 0:
                        ...
                        delete_files(folder_name, num_files_to_delete)
                        st.session_state.deletion_progress[folder_name]["deleted"] = num_files_to_delete

            # Show progress bar for deletion
            if st.session_state.deletion_progress[folder_name]["total"] > 0:
                progress = st.session_state.deletion_progress[folder_name]["deleted"] / st.session_state.deletion_progress[folder_name]["total"]
                st.progress(progress)
                st.write(f"Deleted {st.session_state.deletion_progress[folder_name]['deleted']} out of {st.session_state.deletion_progress[folder_name]['total']} files.")
